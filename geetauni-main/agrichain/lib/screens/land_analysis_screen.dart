...
class _LandAnalysisScreenState extends State<LandAnalysisScreen>
  ...
  Widget _buildAnalyzeButton() {
    return SizedBox(
      ...
      child: ElevatedButton.icon(
        ...
        label: Text(
          _isAnalyzing ? 'Scanning Land...' : 'Analyze Land Quality',
  ...
  Widget _buildMLPredictionsCard() {
    ...
    return _buildCard(
      ...
      child: Column(
        children: [
          _buildMLRow('Crop Quality', ml['crop_quality'] ?? 'N/A', Icons.star),
          _buildMLRow(
            'Health Score',
            '${ml['crop_health_score'] ?? '--'}/100',
            Icons.favorite,
          ),
          _buildMLRow('Risk Level', ml['risk_level'] ?? 'N/A', Icons.shield),
          _buildMLRow(
            'NDVI Trend',
            ml['ndvi_trend'] ?? 'N/A',
            Icons.trending_up,
