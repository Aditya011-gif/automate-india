...
class ContractPdfService {
  ...
  static Future<Uint8List?> _fetchImage(String? url) async {
    ...
    try {
      // Handle Base64 data URIs stored directly in Firestore
      if (url.startsWith('data:image')) {
        final base64Part = url.split(',').last;
        return base64Decode(base64Part);
      }
      // Fallback: fetch from URL (Firebase Storage etc.)
  ...
  static Future<Uint8List> generatePurchaseContract({
    ...
    final buyerSignature =
        buyerSignatureBytes ?? await _fetchImage(buyerSignatureUrl);
      ...
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          return [
            ...
            _buildIndianLegalComplianceSection(
              regularFont,
              boldFont,
              italicFont,
            ),
